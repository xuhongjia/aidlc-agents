---
name: aidlc-enhance
description: 对现有边界内的小增强使用 scope、implement、verify 短流程；每阶段独立子 Agent，保留验收和人工批准。
---
...
只写授权 run 的产物、证据和 result；返回实际 profile/身份/摘要，交父协调器核验并等人批准。高风险或关键歧义返回 blocked，不越级实现。其后 Implement/Verify 使用各自新子 Agent 和对应 Skill，不复用本 child。
