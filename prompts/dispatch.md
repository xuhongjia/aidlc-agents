...
主 Agent 在真实 spawn 时填入下面的任务，不复制当前聊天历史或前一阶段的推理全文。路径必须由当前工作状态解析，不能把未获批准的上游草稿当授权。当前阶段 gate-design leaf 可按 gates.md 接收同阶段冻结草案，仅用于只读设计、不授权实施；需在 input_refs 标明实际来源与摘要。
