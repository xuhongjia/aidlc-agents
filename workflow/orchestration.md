...
| 执行者 | 职责 | 不允许 |
|---|---|---|
| 主 Agent | 读取状态/批准元数据，制作 dispatch，真实 spawn/wait，校验返回，形成 review，向人类汇报并记录决定 | 代跑阶段、伪造子 Agent 身份、把完成返回当批准 |
...
已有安装使用 [update 协议](../bootstrap/update.md)，而不是重新 setup 覆盖。在未结束需求或活动 worker 存在时，默认延后应用升级；仍按旧固定版本完成它们。本版不实现跨版本并存路由或活动工作自动迁移。安装版本变化不能使历史批准失去可解释性，也不能给旧工作偷偷换新执行方式。
