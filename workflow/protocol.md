...
0.9 新工作用 schema 4 和 [扩展 Interface](extensions.md)、[DAG 调度](dag.md)：内置配方也规范化到该接口。下文对三种 profile/单阶段状态/固定正文文件名的描述是内置配方与旧工作解释，不是定制图的调度算法；新工作的真实阶段与产物由 workflow_ref 锁定的 step/kind/语义契约决定。通用身份、原始字节摘要、证据、授权和候选要求仍全量适用。旧工作不迁移。

0.8 新工作同时遵守 [项目知识契约](knowledge.md)：需求来源映射、Verify/Learn 冻结知识、批准后本地晋升与独立发布 Hook。项目知识保存在 system 外；阶段 child 不发布，Hook child 只按限定目标投递固定内容，不授予产品写权限。原阶段/双 Gate/审批/候选冻结规则不变。
