...
先读取 `.aidlc/system/workflow/protocol.md`、`.aidlc/system/workflow/stages.json`、`.aidlc/system/prompts/common.md` 和当前 work 状态。Quality 使用 `.aidlc/system/prompts/quality.md`；Verify 使用 `.aidlc/system/prompts/verify.md`，不要混淆设计与执行。
...
- 发现代码缺陷退回 Implement；发现验收规则错误退回 Spec/Quality，不能边验边改 Oracle 或降低阈值。
